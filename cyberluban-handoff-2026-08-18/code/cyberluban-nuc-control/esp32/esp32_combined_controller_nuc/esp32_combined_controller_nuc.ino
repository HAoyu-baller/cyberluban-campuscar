...
constexpr char FIRMWARE_VERSION[] = "3.0-last-activity-timer";
...
constexpr uint32_t DRIVE_CHANGE_CENTER_HOLD_MS = 80;
...
// The hoverboard's 10-second PWM exit timer is reset whenever either channel
// leaves its 1400-1600 us activity center range. DRIVE_POWER=300 maps to
// 1350/1650 us, so every actual drive output refreshes this timestamp.
uint32_t takeoverLastActivityMs = 0;
bool activationRequested = false;

bool driveRunning = false;
uint32_t driveDeadlineMs = 0;
char activeDriveCommand = 0;
bool driveChangePending = false;
uint32_t driveChangeApplyMs = 0;
int16_t driveChangeSteering = 0;
int16_t driveChangeSpeed = 0;
char driveChangeCommand = 0;
...
void stopDrive() {
  ...
  driveChangePending = false;
  driveChangeCommand = 0;
}

void startOrRefreshDrive(int16_t steering, int16_t speed, char command) {
  if (driveChangePending) {
    // Continue refreshing a direction that is waiting for the short center
    // hold. A different command replaces it before it is applied.
    driveChangeSteering = steering;
    driveChangeSpeed = speed;
    driveChangeCommand = command;
    driveDeadlineMs = millis() + DRIVE_HEARTBEAT_TIMEOUT_MS;
    return;
  }

  if (driveRunning && activeDriveCommand != command) {
    // Some hoverboard PWM receivers need to see neutral between two different
    // commands. Do not jump directly from forward to reverse/turn.
    setCenter();
    driveRunning = false;
    activeDriveCommand = 0;
    driveChangePending = true;
    driveChangeApplyMs = millis() + DRIVE_CHANGE_CENTER_HOLD_MS;
    driveChangeSteering = steering;
    driveChangeSpeed = speed;
    driveChangeCommand = command;
    driveDeadlineMs = millis() + DRIVE_HEARTBEAT_TIMEOUT_MS;
    Serial.print("DRIVE CHANGE CENTER HOLD -> ");
...
bool mayStartMovementNow() {
  ...
  return !timeReached(millis(),
                      takeoverLastActivityMs + TAKEOVER_MOVE_START_LIMIT_MS);
}

bool mayStartGestureNow() {
  return !takeoverCompleted ||
         timeReached(millis(),
                     takeoverLastActivityMs + TAKEOVER_SAFE_REARM_MS);
}
...
void requestActivation() {
  ...
  if (takeoverCompleted && mayStartMovementNow()) {
    // The PWM takeover is still valid. Never replay the activation gesture in
    // this period: its calibration pulses would be interpreted as movement.
    Serial.println("PWM ACTIVE; REARM NOT NEEDED");
    return;
  }
  ...
  if (mayStartGestureNow()) {
    ...
    stopDrive();
    Serial.println("REARM REQUESTED; WAITING FOR SAFE PWM REARM TIME");
  }
}
...
void updateGesture() {
  ...
  if (pendingDrive && !timeReached(millis(), pendingDriveDeadlineMs)) {
    ...
    pendingDrive = false;
  } else if (pendingDrive) {
    pendingDrive = false;
    activationRequested = false;
    Serial.println("PENDING DRIVE EXPIRED; NOTHING STARTED");
  }
}
...
void updateDrive() {
  ...
  if (driveChangePending) {
    if (timeReached(millis(), driveDeadlineMs)) {
      driveChangePending = false;
      driveChangeCommand = 0;
      setCenter();
      Serial.println("SAFETY DRIVE CHANGE HEARTBEAT TIMEOUT");
      return;
    }

    if (timeReached(millis(), driveChangeApplyMs)) {
      setDrive(driveChangeSteering, driveChangeSpeed);
      takeoverLastActivityMs = millis();
      driveRunning = true;
      activeDriveCommand = driveChangeCommand;
      driveChangePending = false;
      Serial.print("DRIVE ");
      Serial.println(activeDriveCommand);
    }
  }
...
void printHelp() {
  Serial.print("READY protocol=3 firmware=");
